...
class StepProgressBar(QWidget):
    ...
    def __init__(self, parent=None, lang="en"):
        ...
        self.setMaximumHeight(52)  # Fixed height for progress bar only
        
        # Main layout for progress bar only (no requirements panel)
        ...
        main_layout.addWidget(self.btn_instruction)
...
class CurriculumCard(QFrame):
    def __init__(self, filename, title, level, icon, color, desc, on_load_click, is_small=False, is_lesson_card=False, on_stop_click=None, start_text="▶ Start", stop_text="⏹ Stop"):
        ...
        title_lbl.setStyleSheet(f"font-weight: bold; font-size: {title_font}px; color: {color}; background: transparent;")
...
class AICodingLab(QMainWindow):
    ...
    def __init__(self):
        ...
        # Model switching shortcuts for perf_test.py
        self._s1 = QShortcut(QKeySequence("Ctrl+1"), self)
        self._s1.activated.connect(lambda: self._send_cmd_to_process("CMD:SWITCH:0"))
        self._s2 = QShortcut(QKeySequence("Ctrl+2"), self)
        self._s2.activated.connect(lambda: self._send_cmd_to_process("CMD:SWITCH:1"))
        self._s3 = QShortcut(QKeySequence("Ctrl+3"), self)
        self._s3.activated.connect(lambda: self._send_cmd_to_process("CMD:SWITCH:2"))
        
        # Toggle shortcut (Shift+Space)
        self._st = QShortcut(QKeySequence("Shift+Space"), self)
        self._st.activated.connect(lambda: self._send_cmd_to_process("CMD:TOGGLE"))
        ...
        # D) REQUIREMENTS PANEL (Separate widget above progress bar)
        from PyQt5.QtWidgets import QSizePolicy
        self._requirements_panel = QFrame()
        ...
        self._requirements_panel.setMaximumHeight(150)
        self._requirements_panel.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Minimum)
        self._requirements_panel.setVisible(False)  # Hidden by default
        
        req_layout = QVBoxLayout(self._requirements_panel)
        ...
        header_layout.addWidget(btn_close_req)
        req_layout.addLayout(header_layout)
        
        # Requirements text
        self._requirements_label = QLabel("SELECT A LESSON TO BEGIN")
        ...
        # Insert requirements panel and progress bar into editorSplitter between editor and console
        # Insert requirements panel and progress bar into editorSplitter between editor and console
        if self.runningEditorSplitter and self.runningEditorSplitter.count() >= 2:
            ...
            if console_widget:
                ...
                self.runningEditorSplitter.addWidget(self._requirements_panel)
                # Add progress bar
                self.runningEditorSplitter.addWidget(self._step_progress_bar)
                # Add console back
                self.runningEditorSplitter.addWidget(console_widget)
                # Set sizes: Editor (large), Requirements (hidden=0), Progress (hidden=0), Console (medium)
                self.runningEditorSplitter.setSizes([600, 0, 0, 200])

        # F) FLOATING INSTRUCTION CARD (overlays the editor)
    ...
    # --- Language Support ---
    def load_requirements_panel(self, lesson_id, step_num):
        """Load and display ONLY the Task section in the separate requirements panel."""
        ...
        self._requirements_label.clear()
        ...
        if os.path.exists(tutorial_file):
            try:
                ...
                # Extract ONLY the Task section
                task_match = re.search(r'## Task\s*\n(.*?)(?=\n##|\Z)', content, re.DOTALL)
                if task_match:
                    task_content = task_match.group(1).strip()
                else:
                    task_content = "No task description available."
                
                # Convert to uppercase and make bold
                task_content = task_content.upper()
                
                # Convert markdown to simple HTML
                task_content = task_content.replace('\n\n', '</p><p style="margin: 4px 0;">')
                task_content = task_content.replace('- ', '<br>• ')
                task_content = task_content.replace('`', '<code style="background: rgba(255,255,255,0.2); padding: 2px 4px; border-radius: 3px; font-weight: bold;">')
                
                # Wrap in bold paragraph
                formatted_content = f'<p style="margin: 4px 0; font-weight: bold;"><strong>{task_content}</strong></p>'
                
                self._requirements_label.setText(formatted_content)
                self._requirements_panel.setVisible(True)
            except Exception as e:
                self._requirements_label.setText(f"<p style='color: #ff6b6b;'>ERROR: {str(e)}</p>")
                self._requirements_panel.setVisible(True)
        else:
            self._requirements_label.setText(f"STEP {step_num} - LESSON {lesson_id}: FILE NOT FOUND")
            self._requirements_panel.setVisible(True)
    
    def hide_requirements_panel(self):
        """Hide the requirements panel."""
        if hasattr(self, '_requirements_panel'):
            self._requirements_panel.setVisible(False)
    
    def show_requirements_panel(self):
    ...
    def refresh_ui_resolution(self, is_transition=False):
        ...
        if hasattr(self, 'training_mode_widget') and self.training_mode_widget:
            ...
            # Direct styling for LOSS metric card (bypass findChildren objectName issues)
            if hasattr(self, 'lblValueLoss') and self.lblValueLoss:
                _mv_size = 10 if is_small else 20
                self.lblValueLoss.setStyleSheet(f"color: #0f172a; font-size: {_mv_size}px; font-weight: bold; background: transparent;")
                # Also style the LOSS title label (sibling in the same layout)
                _loss_parent = self.lblValueLoss.parentWidget()
                if _loss_parent:
                    for _child in _loss_parent.findChildren(QLabel):
                        if _child != self.lblValueLoss:
                            _mt_size = 8 if is_small else 14
                            _child.setStyleSheet(f"color: rgba(0,0,0,0.6); font-size: {_mt_size}px; font-weight: bold; background: transparent;")
            else:
                # Fallback: find LOSS labels by scanning metricsPanel grid column 1
                _metrics_panel = w.findChild(QFrame, "metricsPanel")
                if _metrics_panel:
                    _all_lbl = _metrics_panel.findChildren(QLabel)
                    for _lbl in _all_lbl:
                        if _lbl.text() in ("LOSS", "0.00") or "." in _lbl.text():
                            _mv_size = 10 if is_small else 20
                            _mt_size = 8 if is_small else 14
                            if _lbl.text() == "LOSS":
                                _lbl.setStyleSheet(f"color: rgba(0,0,0,0.6); font-size: {_mt_size}px; font-weight: bold; background: transparent;")
                            else:
                                _lbl.setStyleSheet(f"color: #0f172a; font-size: {_mv_size}px; font-weight: bold; background: transparent;")
            ...
            for field in w.findChildren((QComboBox, QLineEdit)):
                ...
                field.setStyleSheet(f"border: 1px solid #e2e8f0; border-radius: 3px; padding: {_inp_pad}; "
                                  f"background-color: #f8fafc; font-size: {input_size}px; color: #0f172a; min-height: 0px;"
                                  + (f" max-height: 20px;" if is_small else ""))
            ...
            for field in w.findChildren((QSpinBox, QDoubleSpinBox)):
                if is_small:
                    ...
                    field.setStyleSheet(
                        ...
                        f"background-color: #f8fafc; font-size: 8px; color: #0f172a; min-height: 0px; max-height: 20px; }}"
                    )
                else:
                    _spin_h = 32
        ...
        if hasattr(self, '_step_progress_bar') and self._step_progress_bar:
            ...
            if hasattr(self, 'current_lesson_data') and hasattr(self, 'current_step'):
                ...
                if self.current_step <= total_steps:
                    ...
                    if hasattr(self, '_requirements_panel'):
                        self.load_requirements_panel(self.current_lesson_id, self.current_step)
    ...
    def _rebuild_workspace_cards(self):
        ...
        for card, count_name, title, icon, color, bg, border in cards_config:
            if not card:
                continue
            # Clear ALL existing child widgets properly to avoid name collisions (like countCode)
            ...
            old = card.layout()
            if old:
                # Layout is already cleared by deleting children, but we detach it
                import sip
                sip.delete(old)
    ...
    def update_workspace_counts(self):
        ...
        counts = {
            "Code": len([f for f in self.file_manager.code_dir.glob("*.py") if f.is_file()]),
            "Data": len([f for f in self.file_manager.data_dir.glob("*") if f.is_dir()]),
            "Model": len([f for f in self.file_manager.model_dir.glob("*") if f.is_file() and f.suffix.lower() in ['.onnx', '.engine', '.trt', '.h5', '.pt']])
        }
    ...
    def load_lesson_by_id(self, lesson_id_str: str):
        ...
        try:
            ...
            # Show step progress bar and requirements panel
            ...
            if hasattr(self, 'runningEditorSplitter') and self.runningEditorSplitter:
                prog_h = 40 if getattr(self, 'is_small_screen', False) else 52
                req_h = 80 if getattr(self, 'is_small_screen', False) else 100
                # Editor, Requirements, Progress, Console
    ...
    def stop_lesson(self, lesson_id_str: str = None):
        ...
        # Hide step progress bar and requirements panel
        if hasattr(self, '_step_progress_bar'):
        ...
        if hasattr(self, 'runningEditorSplitter') and self.runningEditorSplitter:
            # Editor, Requirements (hidden), Progress (hidden), Console
            self.runningEditorSplitter.setSizes([600, 0, 0, 200])
    ...
    def load_step(self, step_num):
        ...
        try:
            ...
            # Show requirements panel with Task content
            if hasattr(self, '_requirements_panel'):
            ...
            if hasattr(self, '_instruction_card'):
                ...
                # Don't show card automatically, only when user clicks "?"
    ...
    def _send_cmd_to_process(self, cmd):
        """Send a command string to the running child process via stdin."""
        if self._run_process and self._run_process.state() == QProcess.Running:
            self._run_process.write((cmd + "\n").encode())
    ...
    def setup_training_mode_logic(self):
        ...
        self._backbone_map = {
            ...
            "YOLOv26 Nano": "yolo26n.pt"
        }
        self.cmbBackbone = self.training_mode_widget.findChild(QComboBox, "cmbBackbone")
        if self.cmbBackbone:
            # Clear and re-add items to ensure order
            self.cmbBackbone.clear()
            self.cmbBackbone.addItems(list(self._backbone_map.keys()))
        ...
        _vb_fs = 8 if self.is_small_screen else 14
        _vb_h = 16 if self.is_small_screen else 36
        self.btnStopValidation.setVisible(False)
        self.btnStopValidation.setFixedHeight(_vb_h)
        self.btnStopValidation.setStyleSheet(f"""
            QPushButton {{
                background: #ef4444; color: white; border-radius: 8px;
                font-weight: bold; font-size: {_vb_fs}px; padding: {'2px 6px' if self.is_small_screen else '6px 12px'};
            }}
            QPushButton:hover {{ background: #dc2626; }}
        """)
        ...
        self.btnSaveModel.setFixedHeight(_vb_h)
        self.btnSaveModel.setStyleSheet(f"""
            QPushButton {{
    ...
    def _start_training(self):
        ...
        if not backbone_path.exists():
            # Automatically try to pull/download the weight if missing
            self.log_to_console(f"Backbone '{self._selected_backbone}' not found locally. Pulling weights...")
            self.txtTrainLog.appendPlainText(f"> [SYSTEM] Weight '{self._selected_backbone}' missing. Downloading...")
            
            # Use a tiny script to download via Ultralytics
            try:
                import subprocess
                # Run download in a way that respects the training folder
                dl_cmd = [
                    sys.executable, "-c", 
                    f"from ultralytics import YOLO; import shutil; model=YOLO('{self._selected_backbone}'); shutil.move('{self._selected_backbone}', '{backbone_path}')"
                ]
                subprocess.run(dl_cmd, check=True, capture_output=True)
                self.log_to_console(f"Successfully pulled {self._selected_backbone}")
            except Exception as e:
                self.log_to_console(f"Error: Failed to pull backbone model. {e}", is_error=True)
                self.log_to_console(f"Please manually place '{self._selected_backbone}' in src/modules/training/.")
                return
